...
class InventoryExport implements FromView, WithHeadings, WithColumnWidths, WithStyles, WithDefaultStyles, WithCustomStartCell, WithEvents, WithDrawings, WithTitle
{
    ...
    public function view(): View
    {
        ...
        $inventory_fullcounts = FullCount::where([['branch_id', $branch_id], ['period_id', $period_id]])->get();

        $inventory_weights = Weight::where([['branch_id', $branch_id], ['period_id', $period_id]])->get();

        self::$count += count($inventory_fullcounts) + count($inventory_weights);

        return view('admin.report_inventory.exportexcel', [
            'inventory_fullcounts' => $inventory_fullcounts,
