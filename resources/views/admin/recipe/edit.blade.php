...
@section('content')
    ...
    <form class="form-horizontal" action="{{ route('sales.update', $recipe->id) }}" method="POST">
        @csrf
        @method('PUT')
                    ...
                    <div class="col-sm-9" style="display: flex;">
                        ...
                        <div style="margin-left: 15px;margin-right: 40px;margin-top: -5px;">
                            ...
                            @foreach ($stations as $station)
                                    <option value="{{ $station->id }}" {{ old('station_id') && $station->id == old('station_id') ? 'selected' : ($station->id == $station_id ? 'selected' : '') }}>
                                        {{ $station->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <input type="hidden" name="original_station_id" value={{$station_id}}>
                        <div style="margin-top: -5px;">
                            <select name="price_level_id" class="form-control " id="price-level-id">
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-5">
                                        <input type="text" name="plu" value="{{ $recipe->plu }}"
                                ...
                                <div class="form-group prices-container">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-3">
                                            <input 
                                                type="number" 
                                                name="prices[{{ $price_level->id }}]" 
                                                class='form-control'
                                                @foreach($recipe_sales as $data)
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-3">
                                        <input type="number" name="tax" value="{{ $recipe->tax }}" class="form-control"
                                    ...
                                    <div class="form-row ingredient-row align-items-end">
                                        <div class="col-md-4 mb-3">
                                            ...
                                            <select name="item_name[]" class="form-control select2-d item_name" required>
                                                <option value="{{ $ingredient->item_id }}" selected>{{ optional($ingredient->item)->name }}</option>
                                            </select>
                                        </div>
                                        <div class="col-md-2 mb-3">
                                            <label for="name">Quantity</label>
                                            <input type="number" name="qty[]" value="{{ $ingredient->qty }}" class="form-control qty" placeholder="Quantity" required>
                                        </div>
                                        <div class="col-md-2 mb-3">
                                            <label for="name">Unit</label>
                                            <select name="package[]" data-initial="1" data-item_id="{{ $ingredient->item_id }}" class="form-control package" style="width:100%;">
                                            <option value="{{ $ingredient->package_id }}" selected>{{ $ingredient->uom_text }}</option>
                                            </select>
                                            <input type="hidden" class="package_text" name="package_text[]" value="{{ $ingredient->uom_text }}"/>
                                        </div>
                                        <div class="col-md-2 mb-3">
                            ...
                            <div class="card-footer text-right">
                                <button type="submit" class="btn btn-sm btn-primary submit">Submit</button>
...
<script type="text/javascript">
    ...
    var old_station_id='<?php echo(old('station_id') ?? null)?>';
    if(old_station_id){
        var old_price_level_id='<?php echo(old('price_level_id' ?? null)) ?>';
        
        var url=`/ajax/${old_station_id}/get/price_level`;
        $.ajax({
            url:url,
            method: 'GET',
            success:function(result){
                ...
                if(result.price_levels.length){
                    ...
                    $(result.price_levels).each((i, price_level)=>{
                        ...
                        if(price_level.id==old_price_level_id){
                            option+=`<option value="${price_level.id}" selected>${price_level.level}</option>`;
                        }
                        else{
    ...
    $(document).on('change','.item_name', function(){
        getItemSizesPackages($(this), $(this).val())
    })

    $('.package').on('click',function(){
        if($(this).data('initial') == 1){
            $(this).attr('data-initial', 0);
            getItemSizesPackages($(this), $(this).data('item_id'), $(this).val())
        }
    })
    ...
    function getItemSizesPackages($this, itemid, old_value=''){
        $.ajax({
            ...
            success: function(response) {
                ...
                $.each(response.item_size, function(index, item_size) {
                    ...
                    $.each(item_size.item_package, function(idx, item_package) {
                        if (idx == 0) {
                            ...
                            package.append(`<option value="${item_package.id}" ${item_package.id == old_value ? "selected" : ""}>
                                               ${text_option}
                                          </option>`);
                        } else {
                ...
                $.each(uoms, function(key, val) {
                    package.append(`<option value="${key}" ${key == old_value ? "selected" : ""}>
                                    ${val}
                                </option>`);
                });
