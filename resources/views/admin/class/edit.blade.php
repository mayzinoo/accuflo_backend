         ...
         <div class="row">
            <div class="col-12">
               <div class="card">
                  ...
                  <form class="form-horizontal form-validation" action="{{ route('class.update', $class->id) }}"
                     method="POST">
                     @csrf
                     @method('PUT')
                     <div class="card-body">
                        <div class="form-group row">
                           ...
                           <div class="col-md-6">
                              <input type="text" name="name" value="{{ old('name') ? old('name') : $class->name }}"
                           ...
                           <div class="col-md-6">
                              <select name="type" id="type" style="width:100%;">
                                 @foreach ($liquor_types as $liquor_type)
                                    <option value="{{ $liquor_type }}" {{$liquor_type==$class->type? 'selected' : ''}}>{{ $liquor_type }}</option>
                                 @endforeach
                              </select>
                           </div>
                        </div>
                     ...
                     <div class="card-footer text-right">
                        ...
                        <button type="button" class="btn btn-sm btn-default"><a
                              href="{{ url('class') }}">Cancel</a></button>
                     </div>
                  </form>
               </div>
            </div>
